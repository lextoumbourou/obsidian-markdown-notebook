...
### Fixed
- Python background output is unbuffered, output readiness works across stream chunks without mixing stdout and stderr, and readiness failures stop the process with captured diagnostics
- Readiness port checks reject ports already occupied on IPv4 loopback, and stopped processes fully release their port before `stop()` resolves
- Concurrent starts reserve their background name before asynchronous port checks, preventing duplicate and orphaned processes
- Stderr readiness ignores ANSI colour codes, matching the diagnostic text shown in notebook output
